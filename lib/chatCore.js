...
function findAndFeed(socket, broadcastOne) {
    let preFetch = Message.find(broadcastOne ? {'_id': broadcastOne} : {});
    preFetch
        .sort({at: -1})
...
export function submit(socket, data) {
    let message = new Message({
        author: {
            ...
            provider: socket.handshake.session.passport.provider,
            avatar: socket.handshake.session.passport.avatar
        },
        at: Date.now(),
        text: data.text
    });
